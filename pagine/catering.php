...
<?php
    ...
    if(isset($_POST["prenota"]))
    {
        ...
        if(!$PRENOTATO)
        { 
            header("location: checkoutCatering.php?catering=$tipo");
        }
...
<html>
    ...
    <body>
        ...
        <?php
            ...
            {
                $sql = "SELECT * FROM Catering WHERE tipo = '$tipo'";
                $result = $conn -> query($sql);
                if($result -> num_rows > 0)
                {
                    echo '<h2 class="ms-2">'.$titolo.'</h2>';
                                        ...
                                        if(!empty($_SESSION["utente"]))
                                        {
                                            echo'
                                                <form method="post" class="formLogin" action="">
                                                    <input type="submit" name="prenota" style="font-weight:bold;" class="btn btn-primary" value="Prenota">
            ...
            stampa('pesce',$conn,'Pesce');   
            stampa('carne',$conn,'Carne');              
        ?>
        <!--Modal catering-->
        <div class="modal fade" id="catering" tabindex="-1" aria-labelledby="cateringLabel" aria-hidden="true" style="color:black;">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="cateringLabel">Attenzione!</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        Non puoi prenotare più di un catering alla volta!
                    </div>
